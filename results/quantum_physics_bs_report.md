...
**Paper vs. Code — The Dual Track.** The full S-space-to-log-space translation yields both a diffusion addition and a drift correction $-r^2 h^2/(8\sigma^2)$ on the first-derivative term. The shipped implementation **deliberately omits the drift correction**: the code comment explains that "the drift correction is bounded by $O(h^2)$ grid error and validated by `testMilevTaglianiDriftCorrectionAudit()`." This means the shipped operator is a **diffusion-only adaptation** — it does not exactly reproduce the S-space semi-implicit scheme from the paper.
